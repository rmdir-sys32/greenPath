...
function getAbsoluteAQILevel(pm25: number): { label: string; color: string } | null {
    // Only show absolute level if PM2.5 is truly concerning
    if (pm25 > 75) return { label: 'Unhealthy', color: 'bg-red-100 text-red-700 border-red-200' };
    if (pm25 > 55) return { label: 'Unhealthy (S)', color: 'bg-orange-100 text-orange-700 border-orange-200' };
    return null; // Don't show absolute level for good/moderate
}

function getRelativeLabel(index: number, isBest: boolean, pm25: number): { label: string; color: string } {
    if (isBest) {
        // Best route: show as greenest unless it's actually unhealthy
        if (pm25 > 75) return { label: 'Best (but Unhealthy)', color: 'bg-amber-100 text-amber-700 border-amber-200' };
        return { label: 'Greenest', color: 'bg-green-100 text-green-700 border-green-200' };
    }
    // Alternative routes
    if (pm25 > 75) return { label: `Alt ${index} (Unhealthy)`, color: 'bg-red-100 text-red-700 border-red-200' };
    if (pm25 > 35) return { label: `Alt ${index} (Moderate)`, color: 'bg-yellow-100 text-yellow-700 border-yellow-200' };
    return { label: `Alt ${index}`, color: 'bg-gray-100 text-gray-700 border-gray-200' };
}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-left">
                            ...
                            <div className="text-[10px] text-gray-500">
                                Best: {selected.avgPm25.toFixed(1)} µg/m³ • {selected.durationMin.toFixed(0)} min
                            </div>
                        </div>
                    </div>
                    <svg
...
const RouteCard: React.FC<RouteCardProps> = ({ route, isSelected, onSelect }) => {
    const badge = getRelativeLabel(route.index, route.isBest, route.avgPm25);

    return (
        <button
            onClick={onSelect}
            className={`w-full text-left bg-white/95 backdrop-blur-sm rounded-xl shadow-md border-2 transition-all hover:shadow-lg ${isSelected
            ...
            <div className="p-3 space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2 bg-gray-50 rounded-lg px-2 py-1.5">
                    <div className="text-center">
                        ...
                        <div className="text-xs font-bold text-gray-800">{route.avgPm25.toFixed(1)}</div>
                        <div className="text-[8px] text-gray-400">µg/m³</div>
                    </div>
                    <div className="text-center border-x border-gray-200">
                        <div className="text-[10px] text-gray-400">Time</div>
                        <div className="text-xs font-bold text-gray-800">{route.durationMin.toFixed(0)}</div>
                        <div className="text-[8px] text-gray-400">min</div>
                    </div>
                    <div className="text-center">
                        <div className="text-[10px] text-gray-400">Dist</div>
                        <div className="text-xs font-bold text-gray-800">{route.distanceKm.toFixed(1)}</div>
